/**
 * Seeded mock market board + nested column definitions.
 *
 * Column groups are **auto-detected**: any column definition carrying
 * `children` becomes a group header spanning them, so the multi-row header,
 * its collapse/expand affordances and the group state serialization all come
 * from the same column definitions a flat grid uses.
 */
