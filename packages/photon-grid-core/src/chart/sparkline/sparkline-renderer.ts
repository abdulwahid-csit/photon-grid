...
const SparklineTooltip = {
  ...
  _position(el: HTMLElement, cx: number, cy: number): void {
    el.className = 'pg-sparkline-tooltip';
 ...
 */
function cssVar(el: HTMLElement, varName: string, fallback: string): string {
...
export class SparklineRenderer {
  ...
  private readonly isOHLC: boolean;
   ...
   */
  private _resolveConfig(config: SparklineConfig): ResolvedConfig {
   ...
   */
  private _rawHasLabels(rawData: unknown, cfg: ResolvedConfig): boolean {
   ...
   */
  private _parsePoints(rawData: unknown): SparklinePoint[] {
   ...
   */
  private _parseOHLC(rawData: unknown): OHLCPoint[] {
    if (!Array.isArray(rawData) || rawData.length === 0) return [];
  ...
  private _draw(): void {
    ...
    switch (this.cfg.type) {
      case 'line':        this._drawLine(w, h);        break;
      case 'area':        this._drawArea(w, h);        break;
      case 'column':      this._drawColumn(w, h);      break;
      case 'bar':         this._drawBar(w, h);         break;
      case 'win-loss':    this._drawWinLoss(w, h);     break;
      case 'candlestick': this._drawCandlestick(w, h); break;
      case 'ohlc':        this._drawOHLC(w, h);        break;
    }
  }
   ...
   */
  private _drawLine(w: number, h: number): void {
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawArea(w: number, h: number): void {
    const pts = this.points;
    if (pts.length < 2) return;
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawColumn(w: number, h: number): void {
    const pts = this.points;
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawBar(w: number, h: number): void {
    const pts = this.points;
    if (pts.length === 0) return;
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawWinLoss(w: number, h: number): void {
    const pts = this.points;
    if (pts.length === 0) return;
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawCandlestick(w: number, h: number): void {
   ...
   * @param h - CSS logical canvas height.
   */
  private _drawOHLC(w: number, h: number): void {
    const pts = this.ohlcPoints;
    if (pts.length === 0) return;
   ...
   */
  private _xPos(index: number, total: number, plotLeft: number, plotW: number): number {
   ...
   */
  private _yPos(value: number, minV: number, range: number, plotTop: number, plotH: number): number {
