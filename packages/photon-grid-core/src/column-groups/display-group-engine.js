...
export class DisplayGroupEngine {
    constructor(colStyles, eventBus, columnModel, store, iconRenderer) {
         ...
         */
        this.instanceLeaveCache = new Map();
        /**
     ...
     */
    get hasGroups() { return this.registry.hasGroups; }
    /**
     ...
     */
    get isDraggingGroup() { return this.dragHandler?.isDraggingGroup ?? false; }
    /**
     ...
     */
    isFlat(colId) { return this.registry.getPath(colId).length === 0; }
    ...
    toggleGroup(instanceId) {
        ...
        this.eventBus.emit(isNowCollapsed
            ? GridEventType.COLUMN_GROUP_HEADER_COLLAPSED
            : GridEventType.COLUMN_GROUP_HEADER_EXPANDED, { groupId: logicalGroupId });
    }
    /**
    ...
    handleGroupMoved(logicalGroupId, sourcePanel, targetPanel, insertBeforeColId) {
        ...
        const groupLeaves = storeColumns.filter((c) => leafIds.includes(c.colId) && (c.pinned ?? null) === sourcePanel);
    ...
    applyPreviewMove(logicalGroupId, sourcePanel, targetPanel, insertBeforeColId) {
        ...
        if (!baseColumns)
            return;
        const leafIds = this.registry.getLeavesForGroup(logicalGroupId);
        const groupLeaves = baseColumns.filter((c) => leafIds.includes(c.colId) && (c.pinned ?? null) === sourcePanel);
        if (groupLeaves.length === 0)
            return;
        const groupLeafSet = new Set(groupLeaves.map((c) => c.colId));
    ...
    createDragHandler(gridElGetter) {
        if (!this.dragHandler) {
            this.dragHandler = new DisplayGroupDragHandler(gridElGetter, (groupId, src, tgt, before) => this.handleGroupMoved(groupId, src, tgt, before), undefined, // previewCallbacks — CSS-transform path is used (no live store mutations)
    ...
    getGroupPositionInfo(logicalGroupId) {
        ...
        if (!firstLeaf)
            return { isFirst: true, isLast: true, panel: null };
        ...
        const leafIndices = panelCols.reduce((acc, c, i) => {
            if (leafIds.has(c.colId))
                acc.push(i);
            return acc;
        }, []);
        if (leafIndices.length === 0)
            return { isFirst: true, isLast: true, panel };
        return {
    ...
    moveGroupRight(logicalGroupId) {
        ...
        for (let i = 0; i < panelCols.length; i++) {
            if (leafIds.has(panelCols[i].colId))
                lastLeafIdx = i;
        }
        if (lastLeafIdx === -1 || lastLeafIdx >= panelCols.length - 1)
            return;
    ...
    findInstanceLeaves(allCols, firstLeafColId, groupLeafIds) {
        ...
        for (let i = startIdx; i < allCols.length; i++) {
            if (!groupLeafIds.has(allCols[i].colId))
                break;
    ...
    findEndOfPanel(columns, panel) {
        ...
        for (let i = 0; i < columns.length; i++) {
            if ((columns[i].pinned ?? null) === panel)
                lastIdx = i;
        }
        return lastIdx === -1 ? columns.length : lastIdx + 1;
    }
}
//# sourceMappingURL=display-group-engine.js.map