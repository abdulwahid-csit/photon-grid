...
export class GridCore {
    ...
    buildContext(containerEl, options) {
        ...
        const renderer = new GridRenderer(containerEl, store, eventBus, columnModel, paginationEngine, iconRenderer, cellSelectionEngine, sortEngine, rowSelectionEngine, groupingEngine, options);
        // ── Column-group wiring ────────────────────────────────────────────────
        ...
        if (hasGroups) {
            const engine = new DisplayGroupEngine(renderer.colStyles, eventBus, columnModel, store, iconRenderer);
        ...
        renderer.setMasterDetailConfig(masterDetailEngine, (el, opts) => new GridCore(el, opts), iconRenderer, themeManager);
        ...
        masterDetailEngine.setRefreshCallback(() => {
            ...
            if (currentRows.length > 0)
                renderer.captureRowAnimation(currentRows, 'detail');
            this.api.refresh();
        });
        // ── Tree Data wiring ────────────────────────────────────────────────────
        ...
        renderer.setTreeDragConfig(!!options.treeData?.enabled && (options.treeData.mode === 'parentId' || options.treeData.mode === 'childrenField'), (draggedId, targetId, position) => treeDataService.moveNode(draggedId, targetId, position));
    ...
    wireEventHandlers(ctx) {
        ...
        ctx.eventBus.on(GridEventType.FILTER_CHANGED, () => {
            const currentRows = ctx.store.get('visibleRows');
            if (currentRows.length > 0)
                ctx.renderer.captureRowAnimation(currentRows, 'filter');
        });
        ...
        ctx.eventBus.on(GridEventType.COLUMN_GROUP_CHANGED, () => {
            ...
            for (const id of newIds) {
                if (!prevGroupedIds.includes(id))
                    ctx.columnModel.setColumnVisible(id, false);
            }
            for (const id of prevGroupedIds) {
                if (!newIds.includes(id))
                    ctx.columnModel.setColumnVisible(id, true);
            }
        ...
        ctx.eventBus.on(GridEventType.ROW_DETAIL_TOGGLE_CLICKED, (payload) => {
            ...
            const currentRows = ctx.store.get('visibleRows');
            if (currentRows.length > 0)
                ctx.renderer.captureRowAnimation(currentRows, 'detail');
     ...
     */
    wireEditing(ctx) {
        if (ctx.options.editing?.mode === 'none')
            return;
        ...
        const startCellEdit = (payload) => {
            ...
            if (!colDef.editable)
                return;
            if (ctx.cellEditorEngine.isCellEditing(row.nodeId, colDef.colId))
                return;
            // Find the cell and its inner element in the DOM
            const cellEl = ctx.containerEl.querySelector(`[data-node-id="${row.nodeId}"] [data-col-id="${colDef.colId}"]`);
            if (!cellEl)
                return;
            ...
            if (customEditorFn) {
                ...
                const resolvedOpts = colDef.dropdownOptions ??
                    (colDef.enumOptions?.map((v) => ({ value: v, label: v })) ?? []);
                const renderOption = resolveColumnRenderer(colDef, 'option');
                activeDropdown = new CustomDropdownEditor(innerEl, cellEl, resolvedOpts, value, {
                    onSelect: (opt) => {
                        // For 'object' columns store the full option; for 'dropdown' store just the value
                        ctx.cellEditorEngine.updateValue(colDef.type === 'object' ? opt : opt.value);
                    },
                    onStop: (commit) => {
        ...
        ctx.eventBus.on(GridEventType.CELL_EDIT_STOP, () => {
            ...
            activeDropdown = null;
            if (!activeInnerEl || !activeRow || !activeColDef)
                return;
        ...
        const handleTabEdit = (shiftKey) => {
            ...
            if (!activeCell)
                return;
            ctx.cellSelectionEngine.moveActiveCell(0, shiftKey ? -1 : 1, rows.length, cols.length, false);
        ...
        ctx.cellSelectionEngine.setEnterEditHandler((rowIndex, colIndex) => {
            ...
            if (!row || row.type !== 'data' || !colDef || !colDef.editable)
                return false;
        ...
        switch (colDef.type) {
            ...
            case 'object': {
                const key = colDef.type === 'object' && typeof value === 'object' && value !== null
                    ? value[colDef.objectValueKey ?? 'value']
                    : value;
