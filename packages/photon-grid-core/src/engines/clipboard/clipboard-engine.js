...
export class ClipboardEngine {
    ...
    buildRangeData(ranges, rows, columns, withHeaders, leafGroupField) {
        ...
        for (let r = startRow; r <= endRow; r++) {
            ...
            if (isGroupLike && !hasGroupLabel && !row.aggregatedValues)
                continue;
            if (!isGroupLike && row.type !== 'data')
                continue;
    ...
    buildMultiRangeData(ranges, rows, columns, withHeaders) {
        ...
        for (let r = startRow; r <= endRow; r++) {
            ...
            for (let ci = 0; ci < colCount; ci++) {
                const c = startCol + ci;
                const inRange = norms.some((n) => r >= n.startRowIndex && r <= n.endRowIndex &&
                    c >= n.startColIndex && c <= n.endColIndex);
                if (inRange)
                    cells[ci] = fmt(resolve(row.data, descs[ci]), descs[ci]);
            }
            lines.push(cells.join('\t'));
            buffer.push(cells);
        }
...
//# sourceMappingURL=clipboard-engine.js.map