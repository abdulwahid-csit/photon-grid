...
/**
 * Construction contract for {@link FilterEditor}.
 *
 * Unlike {@link FilterPanel}'s config, this carries no positioning/anchor
 * concerns of its own — the editor is a *headless body* that renders into a
 * caller-supplied container. The host (the floating {@link FilterPanel} or a
 * Filters Tool Panel section) owns positioning, footer and dismissal.
 */
export interface FilterEditorConfig {
  /** Column definition for which the filter is being edited. */
  colDef: ColumnDef;
   ...
   * object) columns. Passed in rather than derived here so the caller can cache
   * or sort them once instead of on every render.
   */
  uniqueOptions: FilterSetOption[];
  /** Called immediately when the user changes any filter state (debounced for text inputs). */
  onFilterChange: (filter: ColumnFilter | null) => void;
  /**
   * Anchor element forwarded to a custom filter renderer's
   * {@link FilterRendererParams.anchorEl}. Optional — the inline (tool-panel)
   * host can pass the section header row, the floating panel passes its icon.
   */
  anchorEl?: HTMLElement;
  /**
   * Forwarded as a custom filter renderer's `onClose`, letting a custom filter
   * request that its host shell tear down. Optional — the inline host may leave
   * this unset (its section stays open).
   */
  onRequestClose?: () => void;
}
 ...
 */
export function isSetType(colType: string): boolean {
...
/**
 * Headless, embeddable filter-editing body — the condition/set(checkbox+search)
 * UI shared by the floating {@link FilterPanel} and the Filters Tool Panel's
 * collapsible sections.
 *
 * It owns **only** the filter-editing controls and the assembly of a
 * {@link ColumnFilter} from them; it has no positioning, footer, or dismissal
 * logic — those belong to whatever shell hosts it. This split lets the exact
 * same operator tables, set-filter virtual list, AND/OR logic and
 * model-assembly rules power both the header popup and the tool panel without
 * duplicating a single line.
 *
 * ### Lifecycle
 * ```ts
 * const editor = new FilterEditor(config);
 * editor.render(hostEl);           // builds the body into an already-mounted element
 * // ...user interacts, onFilterChange fires in real-time...
 * editor.setFilter(nextFilter);    // re-hydrate + re-render when the model changes externally
 * editor.destroy();                // clears timers + drops references (never emits)
 * ```
 */
export class FilterEditor {
  /** Host element the body is rendered into — retained so {@link setFilter} can re-render in place. */
  ...
  /**
   * `true` when the built-in condition/set UI is used, `false` when a custom
   * filter renderer owns the body (and therefore its own apply/clear
   * affordances). Hosts use this to decide whether to add a default footer.
   */
  readonly hasDefaultFooter: boolean;

  constructor(private readonly config: FilterEditorConfig) {
    ...
    this.hasDefaultFooter = !resolveColumnRenderer(config.colDef, 'filter');

    this.hydrate(config.currentFilter);
  }

  /**
   * Builds the filter body into `container`. The container **must** already be
   * attached to the live DOM — the set-filter virtual list reads `clientHeight`
   * to size its viewport. Safe to call once per instance; call {@link setFilter}
   * to re-render afterwards.
   *
   * @param container - Host element the editor renders its controls into.
   */
  ...
  /**
   * Re-hydrates internal state from an externally-changed filter and re-renders
   * the body in place. Used when the store's filter model changes underneath an
   * open section (e.g. a header-applied filter, or `clearAllFilters`). Never
   * calls {@link FilterEditorConfig.onFilterChange}, so it cannot cause a
   * feedback loop with whatever applied the change.
   *
   * @param filter - The new applied filter for this column, or `null` when cleared.
   */
  setFilter(filter: ColumnFilter | null): void {
    ...
    if (this.hostEl) {
      ...
      this.buildBody(this.hostEl);
    }
  }

  /** Clears the debounce timer and drops DOM references. Does NOT emit. */
  destroy(): void {
    this.clearDebounce();
    this.hostEl = null;
  ...
  /**
   * Resets condition state so a re-hydrate starts from a clean slate (used by
   * {@link setFilter} before applying a new model, since {@link hydrate} only
   * writes the fields that the incoming filter defines).
   */
  private resetTransientState(): void {
    const ops = getOperators(this.config.colDef.type ?? 'string');
    this.op1 = ops[0].operator;
    this.op2 = ops[0].operator;
    this.val1 = '';
    this.val1To = '';
  ...
  /** Maps an applied {@link ColumnFilter} onto the editor's mutable UI state. */
  private hydrate(cf: ColumnFilter | null): void {
    ...
    if (cf) {
      ...
      this.selectedValues = isSetType(type)
        ? new Set(this.allOptions.map((o) => o.value))
        : new Set();
    }
  }
  ...
  /** Appends the appropriate body (custom / set / condition) into `container`. */
  private buildBody(container: HTMLElement): void {
    const type = this.config.colDef.type ?? 'string';
    ...
    if (customFilterFn) {
      const params: FilterRendererParams = {
        ...
        anchorEl: this.config.anchorEl ?? container,
        currentFilter: this.config.currentFilter,
        uniqueOptions: this.config.uniqueOptions,
        onFilterChange: this.config.onFilterChange,
        onClose: () => this.config.onRequestClose?.(),
        api: null,
      };
      container.appendChild(customFilterFn(params));
      return;
    }

    if (isSetType(type)) {
      container.appendChild(this.buildSetFilter());
      // After insertion, render the virtual list (needs client dimensions).
      if (this.vsContainerEl) this.renderVirtualList();
    } else {
      container.appendChild(this.buildConditionFilter(type));
    }
  }
  ...
  private updateSelectAllState(cb: HTMLInputElement | null): void {
    ...
    const selectedCount = this.setSearchTerm
