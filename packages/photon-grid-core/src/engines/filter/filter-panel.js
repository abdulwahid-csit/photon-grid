...
function getOperators(colType) {
    switch (colType) {
        case 'number':
        case 'currency':
        case 'percentage': return NUMBER_OPERATORS;
        case 'date':
...
export class FilterPanel {
    ...
    buildConditionFilter(colType) {
        ...
        if (!this.hasConditionValue(1))
            logicRow.classList.add('pg-filter-logic--hidden');
        for (const mode of ['and', 'or', 'none']) {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'pg-filter-logic__btn';
            btn.textContent = mode === 'none' ? 'None' : mode.toUpperCase();
            if (this.logicMode === mode)
                btn.classList.add('pg-filter-logic__btn--active');
        ...
        if (this.logicMode === 'none')
            cond2.classList.add('pg-filter-cond__row--hidden');
    ...
    buildConditionRow(colType, index) {
        ...
        input2.addEventListener('input', () => {
            if (index === 1)
                this.val1To = input2.value;
            else
                this.val2To = input2.value;
            this.scheduleEmit();
        });
    ...
    renderVirtualList() {
        if (!this.vsContainerEl || !this.vsInnerEl)
            return;
        ...
        const lastIdx = Math.min(this.displayOptions.length, Math.ceil((scrollTop + containerH) / VS_ITEM_HEIGHT) + VS_BUFFER);
    ...
    buildSetItem(opt, index) {
        ...
        if (optionFn) {
            ...
            if (typeof rendered === 'string')
                ...
                text.appendChild(rendered);
        }
        else {
            text.textContent = opt.label || '(Blank)';
            if (!opt.label)
                text.classList.add('pg-filter-set__item-label--blank');
        }
    ...
    get searchTerm() { return this.setSearchTerm; }
    // ─── Footer ───────────────────────────────────────────────────────────────
    buildFooter() {
    ...
    buildColumnFilter() {
        ...
        if (isSetType(type)) {
            ...
            if (this.selectedValues.size >= this.allOptions.length)
                return null;
    ...
    handleClickOutside(e) {
        ...
        if (this.panelEl?.contains(target) || this.config.anchorEl?.contains(target))
            return;
        this.destroy();
    }
    handleEscapeKey(e) {
