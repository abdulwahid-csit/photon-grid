...
function resolveParentIdEdges(records: Record<string, unknown>[], idField: string, parentIdField: string): TreeEdge[] {
  const edges = new Array<TreeEdge>(records.length);
...
function resolveCustomEdges(
  records: Record<string, unknown>[],
  provider: NonNullable<TreeDataConfig['hierarchyProvider']>,
): TreeEdge[] {
 ...
 * every `refresh()`.
 */
export function flattenNestedChildren(
  rootsRaw: Record<string, unknown>[],
  childrenField: string,
  idField: string,
): TreeEdge[] {
  const edges: TreeEdge[] = [];

  function walk(records: Record<string, unknown>[], parentId: string | null, positionalPrefix: string): void {
    for (let i = 0; i < records.length; i++) {
      const record = records[i];
      ...
      if (Array.isArray(rawChildren) && rawChildren.length > 0) {
        walk(rawChildren as Record<string, unknown>[], id, positionalId);
      }
    }
  }
 ...
 */
export function buildTree(edges: TreeEdge[], defaultRowHeight = 50): RowNode[] {
  const nodesById = new Map<string, RowNode>();
  ...
  const assignLevels = (nodes: RowNode[], level: number): void => {
 ...
 */
export function flattenTree(roots: RowNode[], expandedIds: ReadonlySet<string>): RowNode[] {
  const result: RowNode[] = [];

  const visit = (nodes: RowNode[]): void => {
    for (const node of nodes) {
