...
const selectionInfo = {
    ...
    execute: (_command, api) => {
        ...
        if (rowCount === 0 && cellRanges === 0)
            return 'Nothing is selected.';
        const parts = [];
        if (rowCount > 0)
            parts.push(`${rowCount} row${rowCount === 1 ? '' : 's'}`);
        if (cellRanges > 0)
            parts.push(`${cellRanges} cell range${cellRanges === 1 ? '' : 's'}`);
    ...
    execute: (_command, api) => {
        ...
        const summary = entries
            ...
            if (f.selectedIds?.length)
                return `${header} (${f.selectedIds.join(', ')})`;
...
//# sourceMappingURL=info.commands.js.map