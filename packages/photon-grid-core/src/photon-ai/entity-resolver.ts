...
const BOOLEAN_TRUE_WORDS: ReadonlySet<string> = new Set(['true', 'yes', 'active', 'enabled', 'on', 'open']);
const BOOLEAN_FALSE_WORDS: ReadonlySet<string> = new Set(['false', 'no', 'inactive', 'disabled', 'off', 'closed']);
...
const OPTION_LABEL_STOPWORDS: ReadonlySet<string> = new Set(['of', 'the', 'a', 'an', 'and', 'or']);
...
const OPERATOR_PHRASE_TABLE: readonly OperatorPhrase[] = [
...
const OPERATOR_PHRASES: readonly OperatorPhrase[] = [...OPERATOR_PHRASE_TABLE].sort(
...
function candidateNamesFor(col: ColumnDef): string[] {
  return [col.colId, col.field, col.header].filter((s): s is string => !!s).map(normalizeWord);
}
...
function candidateWordsFor(col: ColumnDef): Set<string> {
...
export class EntityResolver {
  constructor(private readonly memory?: PhotonAIMemoryStore) {}
  ...
  resolveNegation(tokens: readonly string[]): { negated: boolean; remaining: string[] } {
  ...
  resolveRange(tokens: readonly string[]): { from: string[]; to: string[] } | null {
    const words = tokens.map(normalizeWord);
  ...
  parseDateToken(raw: string): Date | null {
    ...
    if (word === 'yesterday') return new Date(now.getTime() - 86_400_000);
    if (word === 'tomorrow') return new Date(now.getTime() + 86_400_000);
  ...
  matchOption(rawValue: string, column: ColumnDef): ColumnDropdownOption | null {
    ...
    const options: ColumnDropdownOption[] =
      column.dropdownOptions ?? column.enumOptions?.map((v) => ({ value: v, label: v })) ?? [];

    let best: ColumnDropdownOption | null = null;
    let bestScore = 0;
  ...
  resolveColumnByValue(tokens: readonly string[], columns: readonly ColumnDef[], api: GridApi): ValueMatch | null {
    ...
    for (const col of filterableColumns) {
      const options: ColumnDropdownOption[] =
        col.dropdownOptions ?? col.enumOptions?.map((v) => ({ value: v, label: v })) ?? [];
      for (const candidate of candidates) {
        for (const option of options) {
          const score = this.optionScore(candidate, String(option.label));
          if (score > bestScore) {
  ...
  private scanRowDataForValue(phrase: string, columns: readonly ColumnDef[], api: GridApi): ValueMatch | null {
  ...
  private scoreCandidate(candidate: string, name: string): number {
   ...
   */
  private optionScore(candidate: string, optionLabel: string): number {
