...
import { applyTreeToggle, syncTreeToggle } from './tree-cell-renderer';
/**
...
export class BodyRenderer {
    ...
    setPanels(leftContent, centerContent, rightContent) {
        ...
        for (const panel of panels) {
            ...
            panel.addEventListener('mouseout', (e) => {
                const nodeId = getNodeId(e.target);
                if (!nodeId || nodeId === getNodeId(e.relatedTarget))
                    return;
                for (const p of panels) {
    ...
    setStickyRows(entries) {
        ...
        for (const nodeId of this.stuckNodeIds) {
            ...
            if (prev) {
                ...
                for (const el of [prev.left, prev.center, prev.right])
                    el?.classList.remove('pg-row--sticky');
            }
        }
        ...
        entries.forEach(({ nodeId, top }, i) => {
            ...
            for (const el of [next.left, next.center, next.right])
                el?.classList.add('pg-row--sticky');
        });
    }
    renderRows(rows, leftCols, centerCols, // visible slice only
    rightCols, options = {}) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (existing && row.type === 'detail') {
                ...
                if (ps.left)
                    leftFrag.appendChild(ps.left);
                if (ps.center)
                    centerFrag.appendChild(ps.center);
                if (ps.right)
                    rightFrag.appendChild(ps.right);
            }
        }
        if (this.leftContent)
            this.leftContent.appendChild(leftFrag);
        this.centerContent?.appendChild(centerFrag);
        if (this.rightContent)
            this.rightContent.appendChild(rightFrag);
    }
    ...
    buildSingleRow(row, displayIndex, panel, cols, colOffset, options) {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            if (options.showVerticalBorders)
                cellEl.classList.add('pg-cell--v-border');
    ...
    buildGroupAggregateCells(el, row, cols, colOffset, options) {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            if (hasValue) {
                ...
                if (align !== 'left')
                    cell.classList.add(`pg-cell--align-${align}`);
                if (options.showVerticalBorders)
                    cell.classList.add('pg-cell--v-border');
    ...
    updatePanelRow(ps, row, displayIndex, options) {
        ...
        for (const el of els) {
            ...
            for (const cell of cells)
                cell.setAttribute('data-row-index', rowIndexStr);
        }
        ...
        if (row.type === 'data' && options.masterDetail) {
            ...
            for (const el of els) {
                ...
                const iconEl = toggleBtn.querySelector('.pg-icon');
                if (iconEl)
                    this.iconRenderer.updateIcon(iconEl, isExpanded ? 'chevronDown' : 'chevronRight');
        ...
        if (row.type === 'data' && options.treeData) {
            for (const el of els)
                syncTreeToggle(el, row, options.treeData, this.iconRenderer);
        }
    }
    /**
    ...
    applyMasterDetailToggle(cellEl, row, colDef, options) {
        ...
        if (!md || row.type !== 'data' || colDef.colId !== md.toggleColumnId)
            return;
    ...
    getRowClass(row, displayIndex, options) {
        const cls = ['pg-row'];
        if (row.selected)
            cls.push('pg-row--selected');
        if (row.type === 'group')
            cls.push('pg-row--group');
        if (row.type === 'group-footer')
            cls.push('pg-row--group-footer');
        if (row.type === 'detail')
            cls.push('pg-row--detail');
        if (options.treeData && row.type === 'data')
            cls.push('pg-row--tree');
        if (row.isTreeFiller)
            cls.push('pg-row--tree-filler');
        if (options.rowShading && displayIndex % 2 === 1)
            cls.push('pg-row--alt');
        if (row.cssClass)
            cls.push(row.cssClass);
        return cls.join(' ');
    }
}
