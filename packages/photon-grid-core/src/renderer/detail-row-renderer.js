...
export class DetailRowRenderer {
    ...
    sync(windowedDetailRows, allDetailNodeIds) {
        if (!this.contentEl || !this.masterDetailEngine)
            return;
        ...
        for (const row of windowedDetailRows) {
            ...
            if (!entry.contentBuilt)
                this.tryBuildContent(entry, row);
        }
    }
    ...
    destroy() {
        for (const entry of this.entries.values())
            this.destroyEntry(entry);
        this.entries.clear();
        for (const entry of this.collapsedCache.values())
            this.destroyEntry(entry);
    ...
    tryBuildContent(entry, row) {
        ...
        if (config?.detailRendererFn) {
            ...
            if (result && result !== entry.containerEl)
                entry.containerEl.appendChild(result);
    ...
    attachWheelForwarding(entry, instance) {
        const handler = (e) => {
            if (e.ctrlKey)
                return; // let pinch-to-zoom through, matching ScrollController.onWheel's own guard
            if (Math.abs(e.deltaX) > Math.abs(e.deltaY))
                return; // horizontal-dominant: let the nested grid scroll itself
            const nestedCanContinue = e.deltaY < 0 ? instance.api.canScrollUp() : instance.api.canScrollDown();
            if (nestedCanContinue)
                return; // let the nested grid's own scroll handle it
            e.preventDefault();
            e.stopPropagation();
...
//# sourceMappingURL=detail-row-renderer.js.map