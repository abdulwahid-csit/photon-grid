...
export class GroupDropZone {
  ...
  private dockPos: DockPosition = 'top';
  private outerRowEl: HTMLElement | null = null;
  private mainColEl: HTMLElement | null = null;

  private isDockDragging = false;
  private dockIndicatorEl: HTMLElement | null = null;
  private _onDockMove: ((e: MouseEvent) => void) | null = null;
  private _onDockUp: ((e: MouseEvent) => void) | null = null;

  private draggingChipId: string | null = null;
  private chipGhostEl: HTMLElement | null = null;
  private chipDropIndex = -1;
  private _onChipMove: ((e: MouseEvent) => void) | null = null;
  private _onChipUp: ((e: MouseEvent) => void) | null = null;
  ...
  isGrouped(colId: string): boolean {
    return (this.store.get('groupedColumnIds') as string[]).includes(colId);
  }
  ...
  private render(): void {
    ...
    const allColumns = this.store.get('columns') as ColumnDef[];
  ...
  private startDockDrag(e: MouseEvent): void {
  ...
  private onDockMove(e: MouseEvent): void {
  ...
  private onDockUp(e: MouseEvent): void {
    if (!this.isDockDragging || !this.el) { this.cleanupDockDrag(); return; }
    const gridEl = this.el.closest('.pg-grid');
    if (gridEl) {
      const gr = gridEl.getBoundingClientRect();
      const pct = (e.clientX - gr.left) / gr.width;
      const newDock: DockPosition = pct < 0.2 ? 'left' : pct > 0.8 ? 'right' : 'top';
  ...
  private startChipDrag(e: MouseEvent, colId: string, index: number): void {
    this.draggingChipId = colId;
    this.chipDropIndex = index;

    const col = (this.store.get('columns') as ColumnDef[]).find((c) => c.colId === colId);
  ...
  private onChipMove(e: MouseEvent): void {
    ...
    const chips = Array.from(
      this.chipsEl.querySelectorAll<HTMLElement>('.pg-group-chip:not(.pg-group-chip--ghost)'),
    );
  ...
  private onChipUp(_e: MouseEvent): void {
  ...
  private cleanupChipDrag(): void {
    this.draggingChipId = null;
    this.chipDropIndex = -1;
    if (this.chipsEl) {
      for (const chip of Array.from(this.chipsEl.querySelectorAll<HTMLElement>('.pg-group-chip'))) {
        chip.style.opacity = '';
      }
    }
