...
export class RowAnimator {
    ...
    animate(containers, newRows, maxDelta) {
        ...
        const duration = this.animationType === 'filter' ? FILTER_DURATION_MS :
        ...
        for (const container of containers) {
            for (const el of container.querySelectorAll('[data-node-id]')) {
                ...
                if (Math.abs(rawDelta) < 1)
                    continue;
                const delta = maxDelta !== undefined && Math.abs(rawDelta) > maxDelta
                    ? Math.sign(rawDelta) * maxDelta
                    : rawDelta;
                toFlip.push({ el, delta });
            }
        }
        if (toFlip.length === 0 && toFadeIn.length === 0)
            return;
        // ── Invert phase ──────────────────────────────────────────────────────
...
//# sourceMappingURL=row-animator.js.map