...
export class TreeStickyRowTracker {
    compute(rows, scrollTop, rowHeight, windowStart) {
        ...
        if (!(candidate.hasChildren || candidate.children.length > 0))
            return none;
        ...
        for (;;) {
            ...
            if (effectiveOffset >= subtreeEnd)
                break; // exhausted this level's subtree — nothing deeper to cascade into
            ...
            if (!(child.hasChildren || child.children.length > 0))
                break; // leaf: no sticky slot here or deeper
            candidate = child;
        }
        return { entries, minStart };
    }
}
//# sourceMappingURL=tree-sticky-row-tracker.js.map