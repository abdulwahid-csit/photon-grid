...
import {
  ...
  type IColumnGroupNode,
  type IColumnLeafNode,
  type ColumnTreeNode,
  type ColumnGroupSerialState,
...
export class ColumnGroupModel {
  ...
  private rootNodes: ColumnTreeNode[] = [];

  /** Quick O(1) lookup by groupId → IColumnGroupNode. */
  private readonly groupMap = new Map<string, IColumnGroupNode>();

  /** Quick O(1) lookup by colId → IColumnLeafNode. */
  private readonly leafMap = new Map<string, IColumnLeafNode>();

  /**
   * Maps any node identifier (groupId or colId) to its immediate parent group.
   * `null` means the node is at the root level.
   */
  private readonly parentMap = new Map<string, IColumnGroupNode | null>();
  ...
  getRootNodes(panel?: 'left' | 'center' | 'right'): ColumnTreeNode[] {
    if (!panel) return this.rootNodes;
    return this.rootNodes.filter((node) => {
      const pin: ColumnPinPosition = node.nodeType === ColumnGroupNodeType.GROUP
        ? (node.pinned ?? null)
        : (node.colDef.pinned ?? null);
      if (panel === 'left')  return pin === 'left';
      if (panel === 'right') return pin === 'right';
  ...
  getAncestors(nodeId: string): IColumnGroupNode[] {
    const path: IColumnGroupNode[] = [];
    let current: IColumnGroupNode | null = this.parentMap.get(nodeId) ?? null;
    while (current !== null) {
  ...
  toggleGroup(groupId: string): void {
    const group = this.groupMap.get(groupId);
    if (!group) return;
    if (group.collapsed) this.expandGroup(groupId);
    else this.collapseGroup(groupId);
  }
  ...
  computeGroupWidth(groupId: string, colStyles: ColumnStyleManager): number {
    ...
    if (group.collapsed) {
      ...
      if (first) return colStyles.getWidth(first.colDef.colId);
  ...
  setLeafVisible(colId: string, visible: boolean): void {
    ...
    if (leaf) leaf.colDef.visible = visible;
  }
  ...
  moveLeaf(colId: string, newParentId: string | null, insertBeforeId: string | null): void {
    ...
    const insertIdx = insertBeforeId !== null
      ...
      : -1;
    if (insertIdx !== -1) newList.splice(insertIdx, 0, leaf);
    else newList.push(leaf);
  ...
  moveGroup(groupId: string, newParentId: string | null, insertBeforeId: string | null): void {
    ...
    const newParent = newParentId ? (this.groupMap.get(newParentId) ?? null) : null;
    if (newParent && this.isAncestorOf(groupId, newParent.groupId)) return;
    ...
    const insertIdx = insertBeforeId !== null
      ? newList.findIndex((n) => this.nodeId(n) === insertBeforeId)
      : -1;
    if (insertIdx !== -1) newList.splice(insertIdx, 0, group);
  ...
  distributeResizeDelta(
    ...
    const siblings = parent.children
      .filter((n): n is IColumnLeafNode =>
        n.nodeType === ColumnGroupNodeType.LEAF &&
  ...
  createSoloGroupForLeaf(colId: string): string | null {
    ...
    const soloGroup: IColumnGroupNode = {
      nodeType:              ColumnGroupNodeType.GROUP,
      groupId:               soloGroupId,
  ...
  dissolveGroupIfSolo(
    ...
    if (!group?.isSoloGroup || group.children.length !== 1) return;

    const leaf = group.children[0];
    if (leaf.nodeType !== ColumnGroupNodeType.LEAF) return;
    ...
    if (insertIdx !== -1) targetList.splice(insertIdx, 0, leaf);
    else targetList.push(leaf);
  ...
  cleanEmptyGroups(): void {
    ...
    for (const [id, group] of this.groupMap) {
      if (group.children.length === 0) toRemove.push(id);
    }
  ...
  serialize(): ColumnGroupSerialState[] {
    const result: ColumnGroupSerialState[] = [];
    for (const [groupId, group] of this.groupMap) {
  ...
  private buildTree(defs: ColumnDef[], parent: IColumnGroupNode | null): ColumnTreeNode[] {
    return defs.map((def) => {
      if (def.children && def.children.length > 0) {
        const groupId = def.colId
          ?? `grp_${def.header.toLowerCase().replace(/[^a-z0-9]+/g, '_')}`;

        const group: IColumnGroupNode = {
          nodeType:        ColumnGroupNodeType.GROUP,
          groupId,
          header:          def.header,
          children:        [],
          ...
          collapsedWidth:  (def as any).collapsedWidth ?? 26,
          resizable:       def.resizable !== false,
          marryChildren:   (def as any).marryChildren === true,
          openByDefault:   def.openByDefault !== false,
          resizeStrategy:  ((def as any).resizeStrategy as ColumnGroupResizeStrategy)
                            ?? ColumnGroupResizeStrategy.PROPORTIONAL,
          headerCssClass:  def.headerCssClass,
          headerRendererFn: (def as any).groupHeaderRendererFn,
        };
      ...
      const leaf: IColumnLeafNode = {
        nodeType: ColumnGroupNodeType.LEAF,
        colDef:   def,
      };
  ...
  private computeDepth(nodes: ColumnTreeNode[]): number {
    let max = 0;
  ...
  private collectLeaves(
    ...
    for (const node of nodes) {
      if (node.nodeType === ColumnGroupNodeType.LEAF) {
        ...
        if (first && first.colDef.visible !== false) result.push(first.colDef);
      }
    }
  }

  private sumLeafWidths(nodes: ColumnTreeNode[], colStyles: ColumnStyleManager): number {
    let total = 0;
    for (const node of nodes) {
      if (node.nodeType === ColumnGroupNodeType.LEAF) {
        ...
        const first = this.findFirstVisibleLeaf(node.children);
        if (first) total += colStyles.getWidth(first.colDef.colId);
      }
    }
    return total;
  }
   ...
   * leaves are hidden.
   */
  private findFirstVisibleLeaf(nodes: ColumnTreeNode[]): IColumnLeafNode | null {
    for (const node of nodes) {
      if (node.nodeType === ColumnGroupNodeType.LEAF) {
        if (node.colDef.visible !== false) return node;
      } else {
  ...
  private nodeId(node: ColumnTreeNode): string {
    return node.nodeType === ColumnGroupNodeType.GROUP ? node.groupId : node.colDef.colId;
  }
   ...
   */
  private isAncestorOf(groupId: string, potentialAncestor: string): boolean {
    let current: string | null = potentialAncestor;
    while (current !== null) {
