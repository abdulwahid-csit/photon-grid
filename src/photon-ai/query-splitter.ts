...
const SOFT_CONNECTORS: ReadonlySet<string> = new Set([',', 'and', 'then', 'also', 'plus']);
 ...
 */
const RANGE_PREFIXES: ReadonlySet<string> = new Set(['between', 'from']);
 ...
 */
const NEVER_SPLIT_AFTER: ReadonlySet<string> = new Set(['select', 'deselect']);
 ...
 * loop iteration.
 */
function isUpcomingVerb(words: readonly string[], atIndex: number, verbs: ReadonlySet<string>): boolean {
 ...
 */
export function splitClauses(raw: string, verbs: ReadonlySet<string>): string[] {
