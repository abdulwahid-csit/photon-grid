...
// Subscribe to every chart event and log it to the panel below.
["chart:created", "chart:destroyed", "chart:rangeSelectionChanged", "chart:optionsChanged"]
...
addButton("Update chart", function () {
  try {
    if (!lastChartId) { log("Create a chart first"); return; }
    // Illustrative options — only apply keys your build supports.
    api.updateChart(lastChartId, { chartType: "bar" });
    log("updateChart(" + lastChartId + ", …) called");
  } catch (err) {
...
log("Interact with the buttons and watch the events appear here.");
