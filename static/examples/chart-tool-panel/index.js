...
// The tool panel edits chart settings; each edit fires chart:optionsChanged.
api.on("chart:optionsChanged", function () {
  log("chart:optionsChanged");
});
...
// updateChart applies the same kind of change the tool panel would make.
addButton("Switch to bar", function () {
  try {
    if (!lastChartId) { log("Create a chart first"); return; }
    api.updateChart(lastChartId, { chartType: "bar" });
    log("updateChart(" + lastChartId + ", { chartType: 'bar' }) called");
  } catch (err) {
    log("updateChart failed: " + err.message);
  }
});

addButton("Switch to line", function () {
  try {
    if (!lastChartId) { log("Create a chart first"); return; }
    api.updateChart(lastChartId, { chartType: "line" });
    log("updateChart(" + lastChartId + ", { chartType: 'line' }) called");
  } catch (err) {
    log("updateChart failed: " + err.message);
  }
});

log("Create a chart, then use Update to change its settings.");
