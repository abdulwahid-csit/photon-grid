...
// Build a normal grid. Enabling selection lets you highlight a range to chart.
...
// Chart ids and models are surfaced through events / return values. We read
// them defensively because the exact payload shape depends on your build.
...
// --- Subscribe to the chart lifecycle event -------------------------------
...
// Create a chart from the currently selected cell range.
addButton("Create range chart", function () {
  try {
    // The config object is illustrative — pass a chart type and a container.
    ...
    if (id) lastChartId = id;
    log("createRangeChart() called — select some salary cells first");
  } catch (err) {
    log("createRangeChart failed: " + err.message);
  }
});

// Create a chart programmatically (no user selection required).
addButton("Create chart", function () {
  try {
    const result = api.createChart({
      chartType: "column",
      chartContainer: document.getElementById("chart")
    });
    const id = readChartId(result);
    ...
    log("createChart failed: " + err.message);
  }
});

log("Ready. Select a range, then click Create range chart.");
