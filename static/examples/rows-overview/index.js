...
function updateCount() {
  countEl.textContent = api.getDisplayedRowCount();
}

let nextId = 9;
const extraNames = ["Liam Walker", "Ava Martin", "Noah Clark", "Mia Lewis"];
const extraDepts = ["Sales", "Engineering", "Finance", "Operations"];

document.getElementById("append").addEventListener("click", function () {
  const i = (nextId - 9) % extraNames.length;
  api.appendData([
    { id: nextId, name: extraNames[i], department: extraDepts[i], country: "USA", salary: 60000 + nextId * 500, active: true }
  ]);
  nextId += 1;
  updateCount();
});
